...
class NansenClient:
    ...
    async def get_historical_holdings(
        self,
        token_address: str,
        chain: str,
        days: int = 30,
        ...
        """
        Get historical smart money holdings for a token over time.

        Args:
            token_address: Token contract address
            chain: Blockchain network
            days: Number of days of history (max 1460 = 4 years)

        Returns:
            List of daily snapshots with date, balance, value_usd, holder_count
        """
        body = {
            "token_address": token_address,
            "chain": chain,
        ...
        cache_key = self._cache.make_key("nansen:historical", chain, token_address, str(days))
        data = await self._post(
    ...
    async def get_token_transfers(
        ...
        """
        Get token transfers by smart money.
        ...
        Returns:
            List of transfer records with from/to addresses, amounts, type (CEX/DEX)
        """
        ...
        cache_key = self._cache.make_key("nansen:tgm:transfers", chain, token_address, str(days))
        data = await self._post(
            "/tgm/transfers",
            body,
            cache_key=cache_key,
