"""
Smart Money Intelligence - Nansen-Only Dashboard
...
def fetch_smart_money_netflow(chains=None, timeframe="24h"):
    ...
    order_field = f"net_flow_{timeframe}_usd" if timeframe != "24h" else "net_flow_24h_usd"
...
@app.route("/")
def index():
    return render_template("nansen/dashboard.html")

@app.route("/token/<chain>/<token_address>")
def token_page(chain, token_address):
    return render_template("nansen/token.html", chain=chain, token_address=token_address)

@app.route("/trader/<chain>/<address>")
def trader_page(chain, address):
    return render_template("nansen/trader.html", chain=chain, address=address)

@app.route("/signals")
def signals_page():
    return render_template("nansen/signals.html")
